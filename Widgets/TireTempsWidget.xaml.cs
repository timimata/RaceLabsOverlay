    ...
    public partial class TireTempsWidget : UserControl, IWidget
    {
        ...
        public void Update(TelemetryData data)
        {
            Dispatcher.Invoke(() =>
            {
                ...
                UpdateTireDisplay(LFInner, LFCenter, LFOuter, LFAvg,
                    data.TireTempLFInner, data.TireTempLFMiddle, data.TireTempLFOuter);

                // Atualizar Right Front
                UpdateTireDisplay(RFInner, RFCenter, RFOuter, RFAvg,
                    data.TireTempRFInner, data.TireTempRFMiddle, data.TireTempRFOuter);

                // Atualizar Left Rear
                UpdateTireDisplay(LRInner, LRCenter, LROuter, LRAvg,
                    data.TireTempLRInner, data.TireTempLRMiddle, data.TireTempLROuter);

                // Atualizar Right Rear
                UpdateTireDisplay(RRInner, RRCenter, RROuter, RRAvg,
                    data.TireTempRRInner, data.TireTempRRMiddle, data.TireTempRROuter);
            });
        }
